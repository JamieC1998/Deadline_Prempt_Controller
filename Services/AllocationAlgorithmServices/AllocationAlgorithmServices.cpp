...
namespace services {
    bool isValidNode(std::chrono::milliseconds estimatedProcTime, float ram_req,
                     float storage_req, std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>  startPoint,
                     std::shared_ptr<model::ComputationDevice> device){

        auto estimatedFinish = estimatedProcTime + startPoint;
        ...
        for (const auto &item: device->getTasks()) {
            if (startPoint <= item->getEstimatedStart() && item->getEstimatedStart() <= estimatedFinish) {
                for (int i = 0; i < 2; i++) {
                    resource_events.push_back(
                            {1, ram_req, storage_req, static_cast<bool>(i),
                             std::chrono::time_point_cast<std::chrono::milliseconds>(static_cast<bool>(i) ? (item->getEstimatedStart() < startPoint)
                                                    ? startPoint
