...
namespace services {
    ...
    [[noreturn]] void NetworkQueueManager::initNetworkCommLoop() {
        ...
        while (true) {
            ...
            if (comms.front()->getCommTime() == std::chrono::high_resolution_clock::now()) {
    ...
    void taskUpdate(model::BaseNetworkCommsModel comm_model, NetworkQueueManager* queueManager) {
    ...
    void taskMapping(model::BaseNetworkCommsModel comm_model, NetworkQueueManager* queueManager) {
        std::shared_ptr<model::BaseResult> br = comm_model.getAllocatedTask();
        std::string hostName = comm_model.getHosts()[0];
    ...
    void haltReq(model::BaseNetworkCommsModel comm_model, NetworkQueueManager* queueManager) {
    ...
    std::shared_ptr<web::json::value> generateTaskJson(model::BaseNetworkCommsModel comm_model, std::string current_block) {
        std::shared_ptr<model::BaseResult> br = comm_model.getAllocatedTask();
