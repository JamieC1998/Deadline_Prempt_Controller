...
//        EXECUTE_BW_TEST
	...
	highTaskAllocation(std::shared_ptr<model::BaseNetworkCommsModel> comm_model,
		...
		if (!is_source_allocation)
			if(br->task_allocation->type == model::contiguous) {
				output["upload_data"] = std::static_pointer_cast<model::LinkAct>(br->task_allocation)->convertToJson();
			}
			else {
				output["upload_data"] = std::static_pointer_cast<model::Bucket>(br->task_allocation)->convertToJson();
			}
