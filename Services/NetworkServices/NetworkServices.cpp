...
namespace services {
    ...
    findLinkSlot(std::chrono::time_point<std::chrono::system_clock> baseStart, float bW_mbpms, float commLatency,
                 float dataSize, std::shared_ptr<std::vector<std::shared_ptr<model::LinkAct>>> netLink) {
        int index = 0;
        float duration = dataSize != 0 ? (dataSize / bW_mbpms) + commLatency : commLatency;
